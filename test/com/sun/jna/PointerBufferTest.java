...
public class PointerBufferTest extends TestCase {
    ...
    public void testByteBufferPutString() {
        ...
        final Memory m = new Memory(1024);
        final ByteBuffer buf = m.getByteBuffer(0, m.size()).order(ByteOrder.nativeOrder());
    ...
    public void testLongBufferPut() {
        ...
        final Memory m = new Memory(8);
        final ByteBuffer buf = m.getByteBuffer(0, m.size()).order(ByteOrder.nativeOrder());
        final LongBuffer lb = buf.asLongBuffer();
        lb.put(MAGIC).flip();
        assertEquals("Long not written to memory", MAGIC,
                m.getLong(0));
    }
    ...
    public void testDirectBufferPointer() throws Exception {
        final Pointer p = new Memory(1024);
        final ByteBuffer b = p.getByteBuffer(0, 1024);
