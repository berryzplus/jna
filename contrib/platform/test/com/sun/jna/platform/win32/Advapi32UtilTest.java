...
public class Advapi32UtilTest extends TestCase {
    ...
    public static void main(final String[] args) {
        ...
        final String currentUserName = Advapi32Util.getUserName();
        System.out.println("GetUserName: " + currentUserName);

        for(final Account group : Advapi32Util.getCurrentUserGroups()) {
			System.out.println(" " + group.fqn + " [" + group.sidString + "]");
		}

		final Account accountByName = Advapi32Util.getAccountByName(currentUserName);
        ...
        final Account accountBySid = Advapi32Util.getAccountBySid(new PSID(accountByName.sid));
	...
	public void testGetUsername() {
		final String username = Advapi32Util.getUserName();
		assertTrue(username.length() > 0);
	}

	public void testGetAccountBySid() {
		final String accountName = Advapi32Util.getUserName();
		final Account currentUser = Advapi32Util.getAccountByName(accountName);
		final Account account = Advapi32Util.getAccountBySid(new PSID(currentUser.sid));
	...
	public void testGetAccountByName() {
		final String accountName = Advapi32Util.getUserName();
		final Account account = Advapi32Util.getAccountByName(accountName);
		assertEquals(SID_NAME_USE.SidTypeUser, account.accountType);
	}
	...
	public void testConvertSid() {
    	final String sidString = "S-1-1-0"; // Everyone
    	final byte[] sidBytes = Advapi32Util.convertStringSidToSid(sidString);
    	assertTrue(sidBytes.length > 0);
    	final String convertedSidString = Advapi32Util.convertSidToStringSid(new PSID(sidBytes));
    	assertEquals(convertedSidString, sidString);
	}

	public void testGetCurrentUserGroups() {
		final Account[] groups = Advapi32Util.getCurrentUserGroups();
		assertTrue(groups.length > 0);
		for(final Account group : groups) {
	...
	public void testGetUserGroups() {
		...
		try {
			...
			try {
				...
				final Account[] groups = Advapi32Util.getTokenGroups(phUser.getValue());
				assertTrue(groups.length > 0);
				for(final Account group : groups) {
					assertTrue(group.name.length() > 0);
					assertTrue(group.sidString.length() > 0);
		...
		try {
			...
			try {
				assertTrue(Advapi32.INSTANCE.LogonUser(userInfo.usri1_name.toString(),
						null, userInfo.usri1_password.toString(), WinBase.LOGON32_LOGON_NETWORK,
						WinBase.LOGON32_PROVIDER_DEFAULT, phUser));
				final Advapi32Util.Account account = Advapi32Util.getTokenAccount(phUser.getValue());
	...
	public void testRegistrySetGetBinaryValue() {
		final byte[] data = { 0x00, 0x01, 0x02 };
		Advapi32Util.registryCreateKey(WinReg.HKEY_CURRENT_USER, "Software", "JNA");
		Advapi32Util.registrySetBinaryValue(WinReg.HKEY_CURRENT_USER, "Software\\JNA", "BinaryValue", data);
		final byte[] read = Advapi32Util.registryGetBinaryValue(WinReg.HKEY_CURRENT_USER, "Software\\JNA", "BinaryValue");
	...
	public void testRegistryGetKeys() {
		...
		final String[] subKeys = Advapi32Util.registryGetKeys(WinReg.HKEY_CURRENT_USER, "Software\\JNA");
	...
	public void testRegistryGetValues() {
		final String uu = new String("A" + "\\u00ea" + "\\u00f1" + "\\u00fc" + "C");
		Advapi32Util.registryCreateKey(WinReg.HKEY_CURRENT_USER, "Software", "JNA");
		...
		final byte[] dataWritten = { 0xD, 0xE, 0xA, 0xD, 0xB, 0xE, 0xE, 0xF };
		Advapi32Util.registrySetBinaryValue(WinReg.HKEY_CURRENT_USER, "Software\\JNA", "DeadBeef", dataWritten);
		final String[] stringsWritten = { "Hello", "World", "Hello World", uu };
		...
		final TreeMap<String, Object> values = Advapi32Util.registryGetValues(WinReg.HKEY_CURRENT_USER, "Software\\JNA");
		...
		final byte[] dataRead = (byte[]) values.get("DeadBeef");
		assertEquals(dataWritten.length, dataRead.length);
	...
	public void testRegistryGetEmptyValues() {
        final HKEY root = WinReg.HKEY_CURRENT_USER;
        final String keyPath = "Software\\JNA";
    ...
    private void doTestRegistryGetEmptyValues(final HKEY root, final String keyPath, final int valueType) {
        final String valueName = "EmptyValue";
        registrySetEmptyValue(root, keyPath, valueName, valueType);
        final Map<String, Object> values = Advapi32Util.registryGetValues(root, keyPath);
        assertEquals(1, values.size());
        assertTrue(values.containsKey(valueName));
    }

    private static void registrySetEmptyValue(final HKEY root, final String keyPath, final String name, final int valueType) {
	...
	public void testIsWellKnownSid() {
		final String everyoneString = "S-1-1-0";
        ...
        final byte[] everyoneBytes = Advapi32Util.convertStringSidToSid(everyoneString);
	...
	public void testEventLogIteratorForwards() {
		final EventLogIterator iter = new EventLogIterator("Application");
		try {
			int max = 100;
			int lastId = 0;
			while(iter.hasNext()) {
				final EventLogRecord record = iter.next();
	...
	public void testEventLogIteratorBackwards() {
		final EventLogIterator iter = new EventLogIterator(null,
	...
	public void testGetEnvironmentBlock() {
		final String expected = "KEY=value\0"
				+ "KEY_EMPTY=\0"
				+ "KEY_NUMBER=2\0"
				+ "\0";

		// Order is important to kept checking result simple
		final Map<String, String> mockEnvironment = new TreeMap<String, String>();
		...
		final String block = Advapi32Util.getEnvironmentBlock(mockEnvironment);
