...
public class Netapi32UtilTest extends TestCase {
    ...
    public static void main(final String[] args) {
		...
		// server local groups
		final Netapi32Util.LocalGroup[] localGroups = Netapi32Util.getLocalGroups();
        System.out.println("Local groups: " + localGroups.length);
		for(final Netapi32Util.LocalGroup localGroup : localGroups) {
			System.out.println(" " + localGroup.name + " (" + localGroup.comment + ")");
		}
		// global groups
		final Netapi32Util.Group[] groups = Netapi32Util.getGlobalGroups();
        System.out.println("Global groups: " + groups.length);
		for(final Netapi32Util.Group group : groups) {
			System.out.println(" " + group.name);
		}
		// server users
		final Netapi32Util.User[] users = Netapi32Util.getUsers();
        System.out.println("Users: " + users.length);
		for(final Netapi32Util.User user : users) {
			System.out.println(" " + user.name);
		}
		// user local groups
		final Netapi32Util.Group[] userLocalGroups = Netapi32Util.getCurrentUserLocalGroups();
        System.out.println("Local user groups: " + userLocalGroups.length);
		for(final Netapi32Util.Group localGroup : userLocalGroups) {
			System.out.println(" " + localGroup.name);
		}
		// domain controller
		if (Netapi32Util.getJoinStatus() == LMJoin.NETSETUP_JOIN_STATUS.NetSetupDomainName) {
			System.out.println("Pdc: " + Netapi32Util.getDCName());
			final DomainController dc = Netapi32Util.getDC();
	...
	public void testGetDomain() {
		...
		final String domain = Netapi32Util.getDomainName(computerName);
	...
	public void testGetUserInfoWithDomainSpecified() {
		final UserInfo userInfo = Netapi32Util.getUserInfo(Advapi32Util.getUserName(),
	...
	public void testGetCurrentUserLocalGroups() {
		final Netapi32Util.Group[] localGroups = Netapi32Util.getCurrentUserLocalGroups();
		assertNotNull(localGroups);
		for(final Netapi32Util.Group localGroup : localGroups) {
			assertTrue(localGroup.name.length() > 0);
		}
		assertTrue(localGroups.length > 0);
	}
	...
	public void testGetJoinStatus() {
		final int joinStatus = Netapi32Util.getJoinStatus();
