...
public class WinspoolTest extends TestCase {
    ...
    public void testEnumPrinters_4() {
    	...
    	if (pcbNeeded.getValue() > 0) {
    		final PRINTER_INFO_4 pPrinterEnum = new PRINTER_INFO_4(pcbNeeded.getValue());
	    	...
	    	assertTrue(pcReturned.getValue() >= 0);
	    	final PRINTER_INFO_4[] printerInfo = (PRINTER_INFO_4[]) pPrinterEnum.toArray(pcReturned.getValue());
	    	for(final PRINTER_INFO_4 pi : printerInfo) {
    ...
    public void testEnumPrinters_1() {
    	...
    	assertTrue(pcReturned.getValue() == 0);
    	if (pcbNeeded.getValue() > 0) {
	    	final PRINTER_INFO_1 pPrinterEnum = new PRINTER_INFO_1(pcbNeeded.getValue());
	    	assertTrue(Winspool.INSTANCE.EnumPrinters(Winspool.PRINTER_ENUM_LOCAL,
	    			null, 1, pPrinterEnum.getPointer(), pcbNeeded.getValue(), pcbNeeded, pcReturned));
	    	assertTrue(pcReturned.getValue() >= 0);
	    	final PRINTER_INFO_1[] printerInfo = (PRINTER_INFO_1[]) pPrinterEnum.toArray(pcReturned.getValue());
	    	for(final PRINTER_INFO_1 pi : printerInfo) {
