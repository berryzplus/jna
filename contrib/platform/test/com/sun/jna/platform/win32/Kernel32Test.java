...
public class Kernel32Test extends TestCase {
    ...
    public static void main(final String[] args) {
    	final OSVERSIONINFO lpVersionInfo = new OSVERSIONINFO();
    ...
    public void testWaitForSingleObject() {
		final HANDLE handle = Kernel32.INSTANCE.CreateEvent(null, false, false, null);
    ...
    public void testWaitForMultipleObjects(){
    	final HANDLE[] handles = new HANDLE[2];
    ...
    public void testGetCurrentThread() {
    	final HANDLE h = Kernel32.INSTANCE.GetCurrentThread();
    	assertNotNull(h);
    ...
    public void testOpenThread() {
    	final HANDLE h = Kernel32.INSTANCE.OpenThread(WinNT.THREAD_ALL_ACCESS, false,
    ...
    public void testGetCurrentProcess() {
    	final HANDLE h = Kernel32.INSTANCE.GetCurrentProcess();
    	assertNotNull(h);
    	assertFalse(h.equals(0));
    ...
    public void testOpenProcess() {
    	final HANDLE h = Kernel32.INSTANCE.OpenProcess(0, false,
    ...
    public void testGetTempPath() {
    	final char[] buffer = new char[WinDef.MAX_PATH];
	...
	public void testGetTickCount() throws InterruptedException {
		...
		// yield a positive.
		final int tick1 = Kernel32.INSTANCE.GetTickCount();
		Thread.sleep(10);
		final int tick2 = Kernel32.INSTANCE.GetTickCount();
		Thread.sleep(10);
		final int tick3 = Kernel32.INSTANCE.GetTickCount();

		assertTrue(tick2 > tick1 || tick3 > tick2);
	}

    public void testGetVersion() {
    	final DWORD version = Kernel32.INSTANCE.GetVersion();
    ...
    public void testGetVersionEx_OSVERSIONINFOEX() {
    	final OSVERSIONINFOEX lpVersionInfo = new OSVERSIONINFOEX();
    	assertEquals(lpVersionInfo.size(), lpVersionInfo.dwOSVersionInfoSize.longValue());
    	assertTrue(Kernel32.INSTANCE.GetVersionEx(lpVersionInfo));
    	assertTrue(lpVersionInfo.dwMajorVersion.longValue() > 0);
    	assertTrue(lpVersionInfo.dwMinorVersion.longValue() >= 0);
    ...
    public void testGetSystemInfo() {
    	final SYSTEM_INFO lpSystemInfo = new SYSTEM_INFO();
    ...
    public void testIsWow64Process() {
    	try {
	    	final IntByReference isWow64 = new IntByReference(42);
	    	final HANDLE hProcess = Kernel32.INSTANCE.GetCurrentProcess();
    ...
    public void testGetLogicalDriveStrings() {
    	final DWORD dwSize = Kernel32.INSTANCE.GetLogicalDriveStrings(new DWORD(0), null);
    ...
    public void testGetDiskFreeSpaceEx() {
    	final LARGE_INTEGER.ByReference lpFreeBytesAvailable = new LARGE_INTEGER.ByReference();
    	final LARGE_INTEGER.ByReference lpTotalNumberOfBytes = new LARGE_INTEGER.ByReference();
    	final LARGE_INTEGER.ByReference lpTotalNumberOfFreeBytes = new LARGE_INTEGER.ByReference();
    ...
    public void testDeleteFile() {
    	final String filename = Kernel32Util.getTempPath() + "\\FileDoesNotExist.jna";
    ...
    public void testReadFile() throws IOException {
    	final String expected = "jna - testReadFile";
    	final File tmp = File.createTempFile("testReadFile", "jna");
    	tmp.deleteOnExit();
    	...
    	final HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_READ, WinNT.FILE_SHARE_READ,
    	...
    	final Memory m = new Memory(2048);
    	final IntByReference lpNumberOfBytesRead = new IntByReference(0);
    	assertTrue(Kernel32.INSTANCE.ReadFile(hFile, m, (int) m.size(), lpNumberOfBytesRead, null));
    	final int read = lpNumberOfBytesRead.getValue();
    ...
    public void testSetHandleInformation() throws IOException {
    	final File tmp = File.createTempFile("testSetHandleInformation", "jna");
    	tmp.deleteOnExit();
    ...
    public void testCreatePipe() {
    	final HANDLEByReference hReadPipe = new HANDLEByReference();
    	final HANDLEByReference hWritePipe = new HANDLEByReference();
    ...
    public void testGetExitCodeProcess() {
    	final IntByReference lpExitCode = new IntByReference(0);
    ...
    public void testTerminateProcess() throws IOException {
    	final File tmp = File.createTempFile("testTerminateProcess", "jna");
    	tmp.deleteOnExit();
    	final HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_READ, WinNT.FILE_SHARE_READ,
    			new WinBase.SECURITY_ATTRIBUTES(), WinNT.OPEN_EXISTING, WinNT.FILE_ATTRIBUTE_NORMAL, null);
    ...
    public void testCopyFile() throws IOException {
        final File source = File.createTempFile("testCopyFile", "jna");
        source.deleteOnExit();
        final File destination = new File(source.getParent(), source.getName() + "-destination");
    ...
    public void testMoveFile() throws IOException {
        final File source = File.createTempFile("testMoveFile", "jna");
        source.deleteOnExit();
        final File destination = new File(source.getParent(), source.getName() + "-destination");
        destination.deleteOnExit();
    ...
    public void testMoveFileEx() throws IOException {
        final File source = File.createTempFile("testMoveFileEx", "jna");
        source.deleteOnExit();
        final File destination = File.createTempFile("testCopyFile", "jna");
        destination.deleteOnExit();
    ...
    public void testCreateProcess() {
        ...
        final WinBase.PROCESS_INFORMATION.ByReference processInformation = new WinBase.PROCESS_INFORMATION.ByReference();

        final boolean status = Kernel32.INSTANCE.CreateProcess(
            null,
            "cmd.exe /c echo hi",
            null,
            null,
    ...
    public void testGetEnvironmentVariable() {
    	...
    	final int size = Kernel32.INSTANCE.GetEnvironmentVariable("jna-getenvironment-test", null, 0);
    	assertTrue(size == 3);
    ...
    public void testGetSetFileTime() throws IOException {
        ...
        final HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_WRITE, WinNT.FILE_SHARE_WRITE,
    			new WinBase.SECURITY_ATTRIBUTES(), WinNT.OPEN_EXISTING, WinNT.FILE_ATTRIBUTE_NORMAL, null);
    	assertFalse(hFile == WinBase.INVALID_HANDLE_VALUE);

        final WinBase.FILETIME.ByReference creationTime = new WinBase.FILETIME.ByReference();
        final WinBase.FILETIME.ByReference accessTime = new WinBase.FILETIME.ByReference();
        final WinBase.FILETIME.ByReference modifiedTime = new WinBase.FILETIME.ByReference();
    ...
    public void testSetFileAttributes() throws IOException {
        final File tmp = File.createTempFile("testSetFileAttributes", "jna");
        tmp.deleteOnExit();

        Kernel32.INSTANCE.SetFileAttributes(tmp.getCanonicalPath(), new DWORD(WinNT.FILE_ATTRIBUTE_HIDDEN));
        final int attributes = Kernel32.INSTANCE.GetFileAttributes(tmp.getCanonicalPath());

        assertTrue((attributes & WinNT.FILE_ATTRIBUTE_HIDDEN) != 0);
    }

    public void testGetProcessList() throws IOException {
        final WinNT.HANDLE processEnumHandle = Kernel32.INSTANCE.CreateToolhelp32Snapshot(Tlhelp32.TH32CS_SNAPALL, new WinDef.DWORD(0));
        ...
        final List<Long> processIdList = new ArrayList<Long>();
