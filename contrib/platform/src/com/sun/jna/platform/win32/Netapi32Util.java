...
public abstract class Netapi32Util {
    ...
    public static String getDCName(final String serverName, final String domainName) {
        ...
        try {
            ...
            if (WinError.ERROR_SUCCESS != Netapi32.INSTANCE.NetApiBufferFree(bufptr.getValue()))
    ...
    public static int getJoinStatus(final String computerName) {
        ...
        try {
            ...
            if (lpNameBuffer.getPointer() != null) {
                final int rc = Netapi32.INSTANCE.NetApiBufferFree(lpNameBuffer.getValue());
                if (LMErr.NERR_Success != rc)
                    throw new Win32Exception(rc);
            }
        }
    ...
    public static LocalGroup[] getLocalGroups(final String serverName) {
        ...
        try {
            final int rc = Netapi32.INSTANCE.NetLocalGroupEnum(serverName, 1, bufptr, LMCons.MAX_PREFERRED_LENGTH, entriesRead, totalEntries, null);
            ...
            final LMAccess.LOCALGROUP_INFO_1[] groups = (LOCALGROUP_INFO_1[]) group.toArray(entriesRead.getValue());

            final ArrayList<LocalGroup> result = new ArrayList<LocalGroup>();
            for(final LOCALGROUP_INFO_1 lgpi : groups) {
            ...
            if (bufptr.getValue() != Pointer.NULL) {
                final int rc = Netapi32.INSTANCE.NetApiBufferFree(bufptr.getValue());
                if (LMErr.NERR_Success != rc)
                    throw new Win32Exception(rc);
            }
        }
    ...
    public static Group[] getGlobalGroups(final String serverName) {
        ...
        final IntByReference totalEntries = new IntByReference();
        try {
            final int rc = Netapi32.INSTANCE.NetGroupEnum(serverName, 1, bufptr,
    ...
    public static User[] getUsers(final String serverName) {
        ...
        final IntByReference totalEntries = new IntByReference();
        try {
            final int rc = Netapi32.INSTANCE.NetUserEnum(
    ...
    public static Group[] getUserLocalGroups(final String userName, final String serverName) {
    	...
    	final IntByReference totalentries = new IntByReference();
    	try {
            final int rc = Netapi32.INSTANCE.NetUserGetLocalGroups(
            		serverName, userName,
            ...
            for (final LOCALGROUP_USERS_INFO_0 lgpi : lgroups) {
                final LocalGroup lgp = new LocalGroup();
    ...
    public static Group[] getUserGroups(final String userName, final String serverName) {
    	...
    	final IntByReference totalentries = new IntByReference();
    	try {
            final int rc = Netapi32.INSTANCE.NetUserGetGroups(
            		serverName, userName,
            ...
            final ArrayList<Group> result = new ArrayList<Group>();
            for (final GROUP_USERS_INFO_0 lgpi : lgroups) {
    ...
    public static DomainTrust[] getDomainTrusts(final String serverName) {
    	...
    	try {
            ...
            for(final DS_DOMAIN_TRUSTS domainTrust : domainTrusts) {
    ...
    public static UserInfo getUserInfo(final String accountName, final String domainName) {
        ...
        try {
            ...
            if (rc == LMErr.NERR_Success) {
                final USER_INFO_23 info_23 = new USER_INFO_23(bufptr.getValue());
