...
public class W32FileMonitor extends FileMonitor {
    ...
    private FileInfo waitForChange() {
        ...
        final ULONG_PTRByReference rkey = new ULONG_PTRByReference();
    ...
    protected synchronized void watch(final File file, final int eventMask, boolean recursive) throws IOException {
        ...
        final Kernel32 klib = Kernel32.INSTANCE;
        final int mask = WinNT.FILE_SHARE_READ
            | WinNT.FILE_SHARE_WRITE | WinNT.FILE_SHARE_DELETE;
        final int flags = WinNT.FILE_FLAG_BACKUP_SEMANTICS
            | WinNT.FILE_FLAG_OVERLAPPED;
        final HANDLE handle = klib.CreateFile(file.getAbsolutePath(),
    ...
    protected synchronized void unwatch(final File file) {
        final FileInfo finfo = fileMap.remove(file);
        if (finfo != null) {
    ...
    public synchronized void dispose() {
        ...
        int i = 0;
        for (final Object[] keys = fileMap.keySet().toArray(); !fileMap.isEmpty();) {
            unwatch((File)keys[i++]);
        }
