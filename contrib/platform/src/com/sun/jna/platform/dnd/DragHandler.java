...
public abstract class DragHandler
    ...
    private final boolean fixCursor = true;
    ...
    protected boolean canDrag(final DragGestureEvent e) {
        final int mods = e.getTriggerEvent().getModifiersEx() & KEY_MASK;
        if (mods == MOVE_MASK)
    ...
    public void dragGestureRecognized(final DragGestureEvent e) {
        if ((e.getDragAction() & supportedActions) != 0
            ...
            final Transferable transferable = getTransferable(e);
            if (transferable == null)
                return;
            try {
                final Point srcOffset = new Point(0, 0);
                final Icon icon = getDragIcon(e, srcOffset);
                ...
                final Icon dragIcon = scaleDragIcon(icon, imageOffset);
                ...
                if (dragIcon != null && DragSource.isDragImageSupported()) {
                    ...
                    if (dragIcon != null) {
                        final Point screen = dragSource.getLocationOnScreen();
                        screen.translate(origin.x, origin.y);
                        final Point cursorOffset = new Point(-imageOffset.x, -imageOffset.y);
    ...
    protected int adjustDropAction(final DragSourceEvent ev) {
        ...
        if (ev instanceof DragSourceDragEvent) {
            final DragSourceDragEvent e = (DragSourceDragEvent)ev;
            if (action == NONE) {
                final int mods = e.getGestureModifiersEx() & KEY_MASK;
                if (mods == 0) {
    ...
    protected void updateCursor(final DragSourceEvent ev) {
        if (!fixCursor)
            return;
        final Cursor cursor = getCursorForAction(adjustDropAction(ev));
